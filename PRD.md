...
| **R-38** | The severity of a verdict must be recoverable from the response without parsing prose. A hard deny that was rerouted to an alternative currently renders as `action: "SAFER ALTERNATIVE"`, indistinguishable from a mild wind override — which the control-room user (§2, P1) consumes programmatically. Add an additive field; do not overload `action`. | **Open** |
| **R-39** | A zone with **no evidence at all** must never resolve to `GO`. Five neutral findings are not five clean bills of health, and "No hazards found; conditions acceptable" from zero observations is exactly the confident-gap failure of §1.3. Such a zone returns a fourth action, **`CANNOT ASSESS`**, carrying an empty `evidence` list and naming the variables it lacked. **Not `DO NOT GO`:** conflating "I know it is dangerous" with "I do not know" is overstated certainty pointed in the safe direction, and a fisherman who learns that `DO NOT GO` sometimes means "no satellite pass today" stops believing `DO NOT GO` at all. *(Open Decision 8, resolved 2026-08-27.)* | **Open** |
| **R-39a** | The `CANNOT ASSESS` verdict is produced in `orca/planner.py`, **not** `orca/policy.py` — the frozen module (N-5) keeps its three rules and its `resolve([])` guard exactly as they are. The planner's test is whether any decision-bearing agent had evidence at that zone, checked before the zone's `Decision` is accepted. | **Open** |
| **R-39b** | A `CANNOT ASSESS` primary zone flows through the planner's **existing** alternative search (R-16): if another zone is a genuine `GO`, the user is told ORCA cannot assess where they asked *and* is offered the real alternative. Inability to assess one zone is not inability to help. | **Open** |
| **R-40** | A verdict must disclose which agents were **blind**, not only which readings were stale. R-33 covers age; nothing yet covers absence, so a `GO` resting on SST alone reads identically to one backed by every source. | **Open** |
...
| **R-45** | Every fallback is **logged with its reason**. A silent fallback is a second untraceable number: a server ran a full day with the layer off and was indistinguishable from one with it on. The API logs its mode at startup for the same reason. *Closed 2026-08-27: the extraction fallback now logs its reason like composition's does; `tests/test_agentic.py::test_extraction_fallback_is_audible` asserts it rather than leaving it to be eyeballed in a terminal. This was the last `except…pass` in the repo, so it also closes N-4.* | **Met** |
...
| **R-49** | The total time the agentic layer may add to one request must be **bounded and stated**. **The bound is `LAYER_BUDGET_S` = 10 s** of network wait per `/ask`, regardless of how many calls the layer makes — down from a ~16 s worst case (two sequential 8 s timeouts), which was precisely the wifi-off stage condition. Enforced two ways, because a check alone is not a bound: composition is **skipped** when the budget is spent, and when it does run its timeout is **clamped to what is left** — without the clamp, extraction finishing just inside the budget still hands composition a full fresh 8 s. A skip is logged, per R-45. | **Met** |
...
| **N-4** | No swallowed exceptions. `except: pass` is prohibited repo-wide, and a typed `except X: pass` must still record why. | **Met** — closed 2026-08-27 with R-45 |
...
| **N-6** | No external call in the `/ask` request path other than the optional, fail-closed agentic call. | **Partial** — see R-54 |
| **N-7** | **Exactly two files may touch the network**: `data/fetch.py` (ahead of time, populating the cache) and `orca/agentic.py` (optional, fail-closed, never touching a marine number). A network call anywhere else is a bug. | **Partial** — see R-54 |
...
| **G-13** | Ask about a coordinate with no cached readings; the answer is `CANNOT ASSESS` with an empty evidence list — never `GO` | The confident gap inside our own deterministic core (R-39) | ⚠ not implemented |
...
| **A zone with no evidence resolves to `GO`.** Five neutral findings reach rule 3 and produce "No hazards found; conditions acceptable" from an empty evidence list. `resolve([])` raising does not prevent this, because the planner never calls it with an empty list. | **Open — highest priority safety defect** | R-39/39a/39b, now fully specified: return `CANNOT ASSESS` from the planner, leave `policy.py` frozen, reroute to a genuine alternative if one exists. Widening `action` also triggers the amended R-25 — `API_CONTRACT.md` and every consumer. Add G-13. This is the §1.3 failure inside our own deterministic core. |
...
| ~~**The extraction fallback is silent.**~~ | **Closed 2026-08-27** | R-45. Both paths now name their reason; verified live with an invalid key and asserted in `tests/test_agentic.py`. Closes N-4. |
| **`_is_reachable()` runs inside `/ask`.** Benign — it cannot change the answer — but it is a network call in the request path, contradicting N-6, N-7 and `orca/api.py`'s own docstring. DNS resolution is not bounded by its 0.75 s timeout. | **Open** | R-54. Move it to `/health`, or state the exception explicitly. |
| ~~**Agentic latency is unbounded at ~16 s worst case.**~~ | **Closed 2026-08-27** | R-49. `LAYER_BUDGET_S` = 10 s for the whole layer: composition is skipped when the budget is spent and clamped to the remainder when it is not. Bounded by `tests/test_agentic.py`, not by comment. |
